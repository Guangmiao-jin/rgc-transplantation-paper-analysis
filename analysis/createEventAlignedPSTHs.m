function [trialHists, trSpikesStruct,  meanStimAlignedTr, edges] = createEventAlignedPSTHs(spikeFrames, fs, binsize, event2Align,  postAlignTime)
% Creates event aligned PSTHs and aligned spike structure based on the
% event in question and a post event time
%
% Inputs: spikeFrames- vector of spike frames for cluster in question
%         fs - recording sampling rate
%         binsize - bin size for histogram binning
%         event2Align - cell array with events split into blocks,
%                       usually 4 x 1
%         postAlignTime - poststimulus aligment time for trials (usually
%                        2 sec)
%
% Outputs: trialHists -  trial by trial histogram with bin size based on
%                        'binsize' input
%          trSpikesStruct - trial by trial spike structure zeroed to
%                             stimON with prestim and poststim time
%          edges - bin edges for plotting the histograms in trialHists

%%
    % for stimblk = 1:length(stimON_Events)-1
    for stimblk = 1:3 % HACK 20250623 MS

        % alignment times in sec
        postAlignInFrames = postAlignTime * fs;

        % for each trial
        for tr = 1:length(event2Align{stimblk})

            % trial times in frames
            trStart = event2Align{stimblk}(tr);
            trEnd = event2Align{stimblk}(tr)+ postAlignInFrames;

            % find inclusions for trial start and end
            trStartIndx = find(spikeFrames >trStart,1, 'first');
            trEndIndx = find(spikeFrames <trEnd,1, 'last');

            % get the spikes
            trSpikes = spikeFrames(trStartIndx:  trEndIndx);
            trSpikes = trSpikes / fs; % convert to sec
            trSpikes = trSpikes- (event2Align{stimblk}(tr)/ fs); % rezero to alignment event

            % put into cell array for psth
            trSpikesStruct{stimblk}{tr} = trSpikes;
            trialLenSec(tr) = (trEnd-trStart)/fs;

            % wipe variables for next trial
            trSpikes = [];
        end

        %% PSTH
        trialLen            = mean(trialLenSec) * 1000;                % trial length ms
        binActual = [0 :(binsize/1000): trialLenSec];
        nbins               = round(trialLen/binsize);                        % Bin duration in [ms]
        nobins              = 1000/binsize;                            % No of bins/sec

        for iTrial = 1:length(trSpikesStruct{stimblk})
            [trialHistTemp, edges] = histcounts(trSpikesStruct{stimblk}{iTrial},binActual);
            % [trialHistTemp, edges] = histcounts(trSpikesStructON{iTrial}, nbins);
            % [trialHistTemp, edges] = histcounts(trSpikesStructON{iTrial},"BinWidth",50);
            trialHists{stimblk}(iTrial,:) = trialHistTemp;
        end

        meanStimAlignedTr{stimblk} = mean(trialHists{stimblk},1) * nobins; % convert  mean PSTH -> spikes/s
 
    end
end